...
namespace MyPersonalDiary.Models
{
    public class Captcha
    {
        ...
        public Captcha()
        {
            ...
            // Потік для збереження зображення
            MemoryStream stream = new MemoryStream();
            bitmap.Save(stream, ImageFormat.Png);

            // Повернення зображення капчі у відповідь
        ...
        private Bitmap GenerateCaptchaImage(string text)
        {
            ...
            using (Graphics graphics = Graphics.FromImage(bitmap))
            {
                ...
                Random random = new Random(); // Створення екземпляру класу Random
                for (int i = 0; i < 10; i++)
                {
                    int x1 = random.Next(0, width); // Використання екземпляру класу Random для генерації випадкових чисел
